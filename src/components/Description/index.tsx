...
// 渲染图片组件
function renderImage(files: any): JSX.Element {
...
// 渲染content
function renderContent<T,>(data: T, term: Term<T>): JSX.Element {
  if (term.render) {
  ...
  if (term.type === 'image')
    return renderImage(data[term.field])
  else if (term.type === 'dictionary')
    return <>dict</>
